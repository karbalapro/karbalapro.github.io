...
export default function ZiyaratListClient({ routeLang }: { routeLang: string }) {
  const { t } = useTranslation();
  const { dir, setLanguage, language } = useLanguage();
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {ziyarats.map((ziyarat, idx) => (
            <Link key={ziyarat.id} href={`/${language}/ziyarat/${ziyarat.id}`}>
              <motion.div
